...
const buildDetailRows = (entry: FormEntry) => [
  ...
  ["Targetted Disposal Date", entry.targetDate],
  ...
  ["Generated By", entry.CreatedBy + " (" + entry.CrBy + ")"],
  // ["Pending with", entry.CreatedBy + " (" + entry.CrBy + ")"],
  ...
  ["Disposed On", entry.ActualDateofDisposal.split("T")[0]]
...
export default function WasteViewPage() {
  ...
  async function GetWasteApproverNameAndDepartment() {
    if (selectedEntry?.stsCode == '2') {
      const res = await fetch('/api/GetData/GetWasteApproverNameAndDepartment', {
        method: "POST",
        body: JSON.stringify({ "ID": selectedEntry.code })
      })
  ...
  useEffect(() => {
    GetWasteApproverNameAndDepartment()
  }, [selectedEntry])

  const [dateOfDisposal, setDateOfDisposal] = useState("")


  async function GetActualDateofDisposalofWaste(ID: any) {
    // console.log(ID)
    const res = await fetch("/api/GetData/GetActualDateofDisposalofWaste", {
      method: "POST",
      body: JSON.stringify({ ID: ID })
    })
    const data = await res.json()
    const disposalDate = await data.data.ActualDateofDisposal
    setDateOfDisposal(disposalDate)
    // console.log(data, disposalDate)
  }
            ...
            <table className="min-w-full text-xs">
              <thead>
                <tr className="text-gray-700 bg-gray-50">
                  ...
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Generation Date</th>
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Waste Category</th>
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Waste</th>
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Quantity</th>
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Generating Department</th>
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Approval Status</th>
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Targetted Disposal Date</th>
                  <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Generated By</th>
                  {/* <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Form 3</th> */}
                  {/* <th className="border border-slate-300 px-2 py-1 text-left font-semibold">Form 10</th> */}
                </tr>
              </thead>
              <tbody className="translate-y-1">
                ...
                {pagedRows.map((item, index) => (
                  <tr
                    ...
                    onClick={() => { setSelectedEntry(item); GetActualDateofDisposalofWaste(item.code) }}
                  >
                    ...
                    <td className="border border-slate-300 px-2 py-1 text-slate-50">{item.quantity}{" "}{item.Unit}</td>
                    <td className="border border-slate-300 px-2 py-1 text-slate-50">{item.genDept}</td>
                    ...
                    <td className="border border-slate-300 px-2 py-1 text-slate-50">{item.CreatedBy} ({item.CrBy})</td>
            ...
            <div className="max-h-[96vh] w-full max-w-5xl overflow-y-auto rounded-xl bg-white p-3 shadow-2xl md:p-6">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2">
                  {selectedEntry.wcid === "1" && (selectedEntry.stsCode != "2" && selectedEntry.stsCode != "5") ? (
                    <>
                      <button
                        type="button"
              ...
              <div className="mt-4 overflow-x-auto rounded-lg border border-slate-300">
                <table className="min-w-full border-collapse text-xs sm:text-sm">
                  ...
                  <tbody>
                    ...
                    {/* <tr key={`${selectedEntry.code}-${dateOfDisposal}`}>
                      <td className="border border-slate-300 px-2 py-2 font-medium text-slate-900">Date of Disposal</td>
                      <td className="border border-slate-300 px-2 py-2 text-slate-800">{dateOfDisposal}</td>
                    </tr> */}
                    {selectedEntry.stsCode == '2' &&
                      <tr key={`${selectedEntry.code}`}>
                        <td className="border border-slate-300 px-2 py-2 text-slate-800">Approval Pending With</td>
                        <td className="border border-slate-300 px-2 py-2 font-medium text-slate-900">{approver[0]?.Dept} {approver.length > 1 ? <span>{" Approvers "} </span> : <span>{" Approver "} </span>}
                          {approver.map((item) => (
                            <div>{item.ApproverName}{" ("}{item.Emp_Code}{")"}</div>
                          ))}
                        </td>
                      </tr>
                    }
