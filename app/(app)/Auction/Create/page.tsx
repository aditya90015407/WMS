...
export default function AuctionList() {
    ...
    useEffect(() => {
        const loadRows = async () => {
            ...
            try {
                const res = await fetch("/api/GetData/GetAllInitiatedAuctionDisposalListToView", {
                    method: "GET",
                    cache: "no-store",
                });
    ...
    const openAuction = async (auctionId: string) => {
        ...
        router.push(`./Create/Act?id=${encodeURIComponent(encryptedID)}`);
    };
        ...
        <section className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="w-full">
                    <h1 className="text-lg font-semibold text-teal-600 text-center">Initiated Auctionable Disposal List</h1>
            ...
            {!loading && !error && filteredRows.length > 0 && (
                <>
                    <p className="mt-4 text-sm text-slate-600">
                        Showing {currentRows.length} of {filteredRows.length} disposals
                    </p>
