...
export default function DisposalApproveHazardousPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
    ...
    const updateEditableField = (key: keyof typeof editableFields, value: string) => {
    ...
    useEffect(() => {
        setEditableFields({
            ReceiverName: String(form10Row?.ReceiverName ?? row?.ReceiverName ?? ""),
            ReceiverAddress: String(form10Row?.ReceiverAddress ?? row?.ReceiverAddress ?? ""),
            ReceiverAuthNo: String(form10Row?.ReceiverAuthNo ?? row?.ReceiverAuthNo ?? ""),
            TransporterName: String(form10Row?.TransporterName ?? row?.TransporterName ?? ""),
            TransporterAddress: String(form10Row?.TransporterAddress ?? row?.TransporterAddress ?? ""),
            TransporterPhone: String(form10Row?.TransporterPhone ?? row?.TransporterPhone ?? ""),
            TransporterEmail: String(form10Row?.TransporterEmail ?? row?.TransporterEmail ?? ""),
            TransporterRegNo: String(form10Row?.TransporterRegNo ?? row?.TransporterRegNo ?? ""),
            VehicleRegNo: String(form10Row?.VehicleRegNo ?? row?.VehicleRegNo ?? ""),
            VehicleType: String(form10Row?.VehicleType ?? ""),
            ManifestDocumentNo: String(form10Row?.ManifestDocumentNo ?? ""),
        });
    }, [form10Row, row]);
                ...
                <>
                    ...
                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                        ...
                        <div className="rounded-xl border border-slate-200 bg-white p-4">
                            ...
                            <div className="mt-3 grid gap-3 text-sm text-slate-700">
                                <div>
                                    <label className="mb-1 block font-medium text-slate-900">Receiver Name</label>
                                    <input
                                        type="text"
                                        value={editableFields.ReceiverName}
                                        onChange={(e) => updateEditableField("ReceiverName", e.target.value)}
                                        className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                    />
                                </div>

                                <div>
                                    <label className="mb-1 block font-medium text-slate-900">Receiver Address</label>
                                    <textarea
                                        rows={3}
                                        value={editableFields.ReceiverAddress}
                                        onChange={(e) => updateEditableField("ReceiverAddress", e.target.value)}
                                        className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                    />
                                </div>

                                <div>
                                    <label className="mb-1 block font-medium text-slate-900">Receiver Auth No.</label>
                                    <input
                                        type="text"
                                        value={editableFields.ReceiverAuthNo}
                                        onChange={(e) => updateEditableField("ReceiverAuthNo", e.target.value)}
                                        className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                    />
                                </div>
                            </div>
                    ...
                    <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-3 grid gap-3 md:grid-cols-2 text-sm text-slate-700">
                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Transporter Name</label>
                                <input
                                    type="text"
                                    value={editableFields.TransporterName}
                                    onChange={(e) => updateEditableField("TransporterName", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>

                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Transporter Address</label>
                                <input
                                    type="text"
                                    value={editableFields.TransporterAddress}
                                    onChange={(e) => updateEditableField("TransporterAddress", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>

                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Transporter Phone</label>
                                <input
                                    type="text"
                                    value={editableFields.TransporterPhone}
                                    onChange={(e) => updateEditableField("TransporterPhone", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>

                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Transporter Email</label>
                                <input
                                    type="text"
                                    value={editableFields.TransporterEmail}
                                    onChange={(e) => updateEditableField("TransporterEmail", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>

                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Vehicle Type</label>
                                <input
                                    type="text"
                                    value={editableFields.VehicleType}
                                    onChange={(e) => updateEditableField("VehicleType", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>

                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Transporter Reg No.</label>
                                <input
                                    type="text"
                                    value={editableFields.TransporterRegNo}
                                    onChange={(e) => updateEditableField("TransporterRegNo", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>

                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Vehicle Reg No.</label>
                                <input
                                    type="text"
                                    value={editableFields.VehicleRegNo}
                                    onChange={(e) => updateEditableField("VehicleRegNo", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>

                            <div>
                                <label className="mb-1 block font-medium text-slate-900">Manifest Document No.</label>
                                <input
                                    type="text"
                                    value={editableFields.ManifestDocumentNo}
                                    onChange={(e) => updateEditableField("ManifestDocumentNo", e.target.value)}
                                    className="w-full rounded border border-slate-300 px-3 py-2 text-sm"
                                />
                            </div>
                        </div>
