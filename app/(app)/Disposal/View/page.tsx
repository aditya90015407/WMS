    ...
    useEffect(() => {
        const loadRows = async () => {
            ...
            try {
                ...
                if (session?.user.roleId != '9') {
                    const res = await fetch("/api/GetData/GetAllInitiatedDisposalListByDept", {
                        method: "POST",
                        cache: "no-store",
                    });
                    ...
                    const res = await fetch("/api/GetData/GetAllInitiatedDisposalList", {
                        method: "POST",
                        cache: "no-store",
                    });
        ...
        <section className="mx-auto max-w-6xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="w-full">
                    <h1 className="text-xl font-semibold text-teal-600 text-center w-full">View All Disposals</h1>
                    ...
                    Original ID
                            ...
                            <tbody className="divide-y divide-slate-100 bg-white">
                                {pagedRows.map((row, index) => (
                                    <tr
                                        key={`${row.IDDID ?? "iddid"}-${index}`}
                                        className="cursor-pointer hover:bg-slate-50"
                                        onClick={async () => {
                                            const encryptedID = await encrypt(row.IDDID!.toString());
