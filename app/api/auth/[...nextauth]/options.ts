  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      if (user) {
        ...
        const roleRow = await getUserRole(String(safeUser.id ?? tokenClaims.sub ?? ""));
        const roleDesignation =
          typeof roleRow?.Role === "string" && roleRow.Role.trim().length > 0
            ? roleRow.Role
            : roleRow?.AssignedRole ?? roleRow?.Desg ?? safeUser.role;
        const resolvedRole = resolveAppRole(roleDesignation);
        const userRole = normalizeRole(resolvedRole.role) ?? normalizeRole(safeUser.role);
        const userRoles = normalizeRoles(resolvedRole.roles) ?? normalizeRoles(safeUser.roles);
        const userRoleId =
          roleRow?.RoleID !== null && roleRow?.RoleID !== undefined
            ? String(roleRow.RoleID)
            : safeUser.roleId !== null && safeUser.roleId !== undefined
              ? String(safeUser.roleId)
              : undefined;
        const userRoleName =
          typeof roleRow?.Role === "string" && roleRow.Role.trim().length > 0
            ? roleRow.Role
            : typeof safeUser.roleName === "string"
              ? safeUser.roleName
              : undefined;
