...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const mailBody = `
        <p>Dear Store Approver,</p>

        <p>The Vendor selected for Auction ${IDDID} has accepted the awarded auction.</p>

        <p><strong>Auction Details:</strong></p>
        <ul>
          <li>ID : ${IDDID}</li>
          <li>Waste : ${Waste}</li>
          <li>Total Quantity : ${Quantity} ${MUnit}</li>
          <li>Awarded to : ${EmpName} (${EmpCode})</li>
          <li>Accepted On : ${generatedOn}</li>
        </ul>
        ...
        const sendMail = await fetch(`https://jajitapps.jindalstainless.com:9234/api/AutoEmail/InstantEmailSend`, {
            ...
            body: JSON.stringify({
                "subject": `WMS : Vendor Accepted Auction_${IDDID}_Action Required`,
                "emailBody": mailBody,
